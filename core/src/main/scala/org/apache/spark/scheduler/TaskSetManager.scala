...
private[spark] class TaskSetManager(
  ...
  private def getAllowedLocalityLevel(curTime: Long): TaskLocality.TaskLocality = {
    ...
    while (currentLocalityIndex < myLocalityLevels.length - 1) {
      ...
      if (!moreTasks) {
        ...
        //dschatzl
        logInfo("CURRENT WAIT,NODE_LOCAL: [" + localityWaits(currentLocalityIndex) + "," + getLocalityWait(TaskLocality.NODE_LOCAL) + "]")
  ...
  def handleSuccessfulTask(tid: Long, result: DirectTaskResult[_]): Unit = {
    ...
    //dschatzl
    val my_metrics = result.metrics
    logInfo("RECORD READ TIME = [" + my_metrics.netOH + "]")
    ...
    if(getLocalityWait(TaskLocality.NODE_LOCAL) == 3001){
        logInfo("UPDATING");
        while(my_index < myLocalityLevels.length - 1){
            localityWaits.update(my_index,delay_average);
            my_index += 1;
        }
    }
